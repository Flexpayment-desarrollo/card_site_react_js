import '../../../../EasyCardClientes/src/Global/Constants';
...
export async function emailValidate(form, latitud, longitud) {
   ...
   return new Promise((resolve, reject) => {
      ...
      axios.post(global.Constants.url + global.Constants.port + '/auth/validate', data,
         {
            headers: {
...
export async function authValidate(form) {
   ...
   return new Promise((resolve, reject) => {
      const axios = require('axios');
      axios.post(global.Constants.url + global.Constants.port + '/auth/login',
         formLogin,
         {
            headers:
            {
         ...
         .then((response) => {
            if (response.data.code === 0) {
               ...
               var Authentication = JSON.stringify(response.data.data);
